...
28. [Двухвыборочный перестановочный критерий, гипотеза о средних, независимые и связанные выборки. Пример.](#28-двухвыборочный-перестановочный-критерий-гипотеза-о-средних-независимые-и-связанные-выборки-пример)
...
- **Групповая дисперсия** ($SS_{between}$): Эта дисперсия измеряет вариацию между группами. Она показывает, насколько средние значения различных групп отличаются друг от друга.
- **Внутригрупповая дисперсия** ($SS_{within}$): Эта дисперсия измеряет вариацию внутри каждой группы. Она показывает, насколько данные внутри каждой группы отличаются от среднего значения этой группы.
- **Межгрупповая дисперсия** ($SS_{between}$): Она аналогична групповой дисперсии и используется для обозначения той же вариации между группами.
- **Общая дисперсия** ($SS_{total}$): Это сумма групповой и внутригрупповой дисперсий. Она измеряет общую вариацию данных.

#### Формулы

1. **Общая дисперсия (Total Sum of Squares, $SS_{total}$):**
   $$
   SS_{total} = \sum_{i=1}^{N} (X_i - \bar{X})^2
   $$

   где $N$ - общее число наблюдений, $X_i$ - каждое наблюдение, $\bar{X}$ - общее среднее значение.

2. **Межгрупповая дисперсия (Between Groups Sum of Squares, $SS_{between}$):**
   $$
   SS_{between} = \sum_{j=1}^{k} n_j (\bar{X}_j - \bar{X})^2
   $$

   где $k$ - число групп, $n_j$ - число наблюдений в группе $j$, $\bar{X}_j$ - среднее значение группы $j$, $\bar{X}$ - общее среднее значение.

3. **Внутригрупповая дисперсия (Within Groups Sum of Squares, $SS_{within}$):**
   $$
   SS_{within} = \sum_{j=1}^{k} \sum_{i=1}^{n_j} (X_{ij} - \bar{X}_j)^2
   $$

   где $X_{ij}$ - каждое наблюдение в группе $j$, $\bar{X}_j$ - среднее значение группы $j$.

4. **Связь между дисперсиями:**
   $$
...
Предположим, у нас есть три группы студентов, проходящих тест на время реакции. Мы хотим определить, различаются ли их средние времена реакции.
...
2. **Вычисление общих средних:**

   $$
   \bar{X}_1 = \frac{20 + 22 + 19 + 24 + 20}{5} = 21
   $$

   $$
   \bar{X}_2 = \frac{28 + 30 + 27 + 31 + 29}{5} = 29
   $$

   $$
   \bar{X}_3 = \frac{22 + 24 + 19 + 23 + 21}{5} = 21.8
   $$

   $$
   \bar{X} = \frac{21 + 29 + 21.8}{3} = 23.27
   $$
...
4. **Вычисление внутригрупповой дисперсии:**

   $$SS_{within} = \sum_{j=1}^{3} \sum_{i=1}^{5} (X_{ij} - \bar{X}_j)^2$$

   После вычислений получаем:

   $$SS_{within} = 4 + 3.2 + 4.8 = 12$$

5. **Вычисление общей дисперсии:**

   $$SS_{total} = SS_{between} + SS_{within} = 153.35 + 12 = 165.35$$

### [Про всякие дисперсии](http://mathprofi.ru/dispersii.html)
...
### Основные шаги проведения MANOVA

1. **Определение гипотез:**
   - Нулевая гипотеза ($H_0$): Средние значения зависимых переменных одинаковы для всех групп.
   - Альтернативная гипотеза ($H_1$): По крайней мере, одна группа имеет среднее значение, отличное от других.

2. **Сбор данных:**
   - Сбор данных по зависимым переменным для различных уровней факторов.

3. **Проверка предпосылок:**
   - Нормальность распределения зависимых переменных.
   - Равенство ковариационных матриц (проверка с помощью теста Бартлетта).

4. **Построение модели:**
   - Определение зависимых переменных и факторов.
   - Формирование модели многофакторного дисперсионного анализа.

5. **Вычисление дисперсий:**
   - Вычисление групповой, внутригрупповой, межгрупповой и общей дисперсий.

6. **Анализ результатов:**
   - Интерпретация значений статистики (например, статистики Wilks' Lambda).
   - Оценка значимости факторов и их взаимодействий.

### Групповая, внутригрупповая, межгрупповая и общая дисперсии

1. **Групповая дисперсия (Between Groups Sum of Squares, $SS_{between}$):**

   $$SS_{between} = \sum_{j=1}^{k} n_j (\bar{X}_j - \bar{X})^2$$

   где $k$ - число групп, $n_j$ - число наблюдений в группе $j$, $\bar{X}_j$ - среднее значение группы $j$, $\bar{X}$ - общее среднее значение.

2. **Внутригрупповая дисперсия (Within Groups Sum of Squares, $SS_{within}$):**

   $$SS_{within} = \sum_{j=1}^{k} \sum_{i=1}^{n_j} (X_{ij} - \bar{X}_j)^2$$

   где $X_{ij}$ - каждое наблюдение в группе $j$, $\bar{X}_j$ - среднее значение группы $j$.

3. **Межгрупповая дисперсия (Between Factors Sum of Squares, $SS_{factors}$):**

   $$SS_{factors} = \sum_{l=1}^{f} n_l (\bar{X}_l - \bar{X})^2$$

   где $f$ - число факторов, $n_l$ - число наблюдений для фактора $l$, $\bar{X}_l$ - среднее значение для фактора $l$, $\bar{X}$ - общее среднее значение.

4. **Общая дисперсия (Total Sum of Squares, $SS_{total}$):**

   $$SS_{total} = \sum_{i=1}^{N} (X_i - \bar{X})^2$$

   где $N$ - общее число наблюдений, $X_i$ - каждое наблюдение, $\bar{X}$ - общее среднее значение.

5. **Связь между дисперсиями:**
   $$SS_{total} = SS_{between} + SS_{within}$$

#### Пример

Предположим, у нас есть данные о времени реакции на стимулы для различных групп испытуемых, где учитываются два фактора: тип стимула (А, В, С) и время суток (утро, день, вечер).

1. **Данные:**

   - Тип стимула: А, В, С
   - Время суток: утро, день, вечер
   - Время реакции измеряется в миллисекундах для каждой комбинации факторов.

2. **Вычисление средних значений:**

   $$\bar{X}_{A, утро} = \frac{\sum_{i=1}^{n_{A, утро}} X_{i, A, утро}}{n_{A, утро}}$$

   и так далее для каждой комбинации факторов.

3. **Вычисление дисперсий:**

   - **Межгрупповая дисперсия:**

     $$SS_{between} = \sum_{типов стимула} \sum_{время суток} n_{j, l} (\bar{X}_{j, l} - \bar{X})^2$$

   - **Внутригрупповая дисперсия:**

     $$SS_{within} = \sum_{типов стимула} \sum_{время суток} \sum_{i=1}^{n_{j, l}} (X_{ij, l} - \bar{X}_{j, l})^2$$
...
4. **Результаты:**
   ...
   После вычислений дисперсий и выполнения многофакторного дисперсионного анализа (MANOVA), интерпретируются результаты с целью определения значимости факторов и их взаимодействий.
